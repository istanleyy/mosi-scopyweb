...
def processServerAction(data):
    ...
    if actparam[0] == 'co':
        ...
        print 'Reset device<{}> output counter. Current={}'.format(id(device), device._total_output)
        device._total_output = 0
        print 'Updated output counter={}'.format(device._total_output)
        return True
    else:
        return False
